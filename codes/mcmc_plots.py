...
# Default parameter labels for linear sigma
DEFAULT_PARAM_LABELS_LINEAR = [
    'w',
    r'$\mu_1$',
    r'$\delta = \mu_2 - \mu_1$',
    r'$\sigma_1$',
...
def plot_marginal_distributions(
    ...
    sigma_scale : str, optional
        'ln' (default) to plot ln(sigma) (no transform),
        'linear' to exponentiate sigma columns and plot sigma.
    ...
    axes : list of Axes
    """
    if sigma_scale not in ("ln", "linear"):
        raise ValueError("sigma_scale must be 'ln' or 'linear'")
        
    # Choose default parameters if none provided
    if param_tuples is None:
        # use up to the first 5 default labels
        labels = DEFAULT_PARAM_LABELS_LN if sigma_scale == "ln" else DEFAULT_PARAM_LABELS_LINEAR
        param_tuples = [(i, labels[i]) for i in range(min(5, samples.shape[1]))]
    ...
    for col, ((idx, title), color) in enumerate(zip(param_tuples, colors)):
        ...
        if sigma_scale == "linear" and idx in (3, 4):
            samples_dist = np.exp(samples[:, idx])
        else:
...
def pairplot(
    ...
    for i in range(k):
        ...
        if sigma_scale == "linear" and idx_i in (3, 4):
            xi = np.exp(samples[:, idx_i])
        else:
            xi = samples[:, idx_i]
        #xi = samples[:, idx_i]
        ...
        for j in range(0, i + 1):  # include diagonal
            ...
            if i == j:
                ...
                if theta_true is not None:
                    true_val = float(theta_true[idx_i])
                    if sigma_scale == "linear" and idx_i in (3, 4):
                        true_val = np.exp(true_val)
                    ax.axvline(true_val, color='red', linestyle='-', linewidth=1.6)
            ...
            # off-diagonal lower triangle: scatter 
                ...
                if sigma_scale == "linear" and idx_j in (3, 4):
                    xj = np.exp(samples[:, idx_j])
                else:
                    xj = samples[:, idx_j]
                #xj = samples[:, idx_j]
                ...
                if theta_true is not None:
                    true_value_i = float(theta_true[idx_i])
                    true_value_j = float(theta_true[idx_j])
                    if sigma_scale == "linear" and idx_i in (3, 4):
                        true_value_i = np.exp(true_value_i)
                    if sigma_scale == "linear" and idx_j in (3, 4):
                        true_value_j = np.exp(true_value_j)    
                        
                    ax.axvline(true_value_j, color='red', linestyle='-', linewidth=1.6)
                    ax.axhline(true_value_i, color='red', linestyle='-', linewidth=1.6)
...
def plot_trace_chains(
    ...
    save: bool = True,):
    """
    Semplice funzione per disegnare trace plots delle catene MCMC.

    Parameters
    ----------
    chains : ndarray
         ndarray of shape (n_chains, n_steps, d).
    filename: str
        Name of the file
    theta_true : np.ndarray
        1D array with the "true" or reference values for each parameter. Length must equal n_parameters.
    ...
    burnin : int or None, optional
        ...
        If None, no vertical burn-in line is drawn. Default: 3000.
    figsize : tuple
        Figura size in inches.
    alpha : float
        Transparency of the chains.
    linewidth : float
        Width of the chains.
    outdir : str, optional
        Directory where PNG/PDF files will be saved if `save` is True.
    ...
    -------
    fig, axes
        Matplotlib Figure e lista di Axes (uno per parametro).
    """

    # Normalizza input in array (n_chains, n_steps, d)
    arr = chains
    if chains.ndim == 2:
        # un'unica chain: (n_steps, d) -> (1, n_steps, d)
        chains = chains[np.newaxis, ...]
    elif chains.ndim == 3:
        pass
    else:
        raise ValueError("`chains` ndarray must be 2D (n_steps,d) or 3D (n_chains,n_steps,d)")

    n_chains, n_steps, d = chains.shape

    labels = list(param_labels) if param_labels is not None else DEFAULT_PARAM_LABELS[:d]
    ...
    # Plot the first parameter and collect line handles for the legend
    handles = []
    x = np.arange(n_steps)
    for k in range(n_chains):
        h, = axes[0].plot(x, arr[k, :, 0], label=f'chain {k+1}', alpha=alpha, linewidth=linewidth)
        handles.append(h)
    if theta_true is not None:
        axes[0].axhline(theta_true[0], label='real', color='lime', linestyle='--', linewidth=1.8)
    if burnin is not None:
        axes[0].axvline(burnin, color='grey', label='burnin', linestyle=':', linewidth=1.8)

    # Plot remaining parameters (no labels to avoid duplicate legend entries)
    for i in range(1, d):
        for k in range(n_chains):
            axes[i].plot(x, arr[k, :, i], alpha=alpha, linewidth=linewidth)
        if theta_true is not None:
            axes[i].axhline(theta_true[i], color='lime', linestyle='--', linewidth=1.8)
        if burnin is not None:
            axes[i].axvline(burnin, color='grey', linestyle=':', linewidth=1.8)
    ...
    # Legend above the first plot, split in two rows
    ncol = math.ceil(n_chains / 2)
    # Place legend relative to the axes: center above the first axes. Adjust top margin so it fits.
    axes[0].legend(handles=handles, loc='lower center', bbox_to_anchor=(0.5, 0.95),
                   ncol=ncol, frameon=False)
    fig.subplots_adjust(top=0.88, hspace=0.3)

    fig.suptitle("Chains trace plot", fontsize=16)

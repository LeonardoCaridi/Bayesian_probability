...
def estimate_covariance(theta0, x, bounds, rng=None,
    ...
    burnin : int, optional
        Number of initial adaptive samples to discard when computing the sample
        covariance. The covariance (and cov_history entries) are computed using
        `samples[burnin:]`. Default 0 (no burnin).
    ...
    eps : float, optional
        Small number added to covariance estimates to ensure positive-definiteness.
...
#        accept_flags.append(accepted_flag)
        ...
        if (t % adapt_interval) == 0:
            arr_all = np.asarray(samples)   # shape (n_samples, d)
            # samples after burnin
            arr = arr_all[burnin:] if burnin < arr_all.shape[0] else np.empty((0, d))

            if arr.shape[0] > 1:
                # compute sample covariance and add eps 
                cov_new = np.cov(arr, rowvar=False, bias=False)
                cov_new = cov_new + eps * np.eye(d)   # add eps 
            else:
                cov_new = cov_est.copy()

            if arr.shape[0] > 1:
                # compute sample covariance once and add eps exactly here
                cov_new = np.cov(arr, rowvar=False, bias=False)
                cov_new = cov_new + eps * np.eye(d)   # add eps 
            else:
                cov_new = cov_est.copy()
...
#            overall_acc = float(sum(accept_flags)) / float(len(accept_flags))
            ...
            if len(cov_history) >= 2 and t >= min_adapt:
                ...
                if verbose:
                    overall_acc = accepted / t
                    print(f"[adapt t={t}] overall_acc={overall_acc:.3f} "
...
#    overall_acc = float(sum(accept_flags)) / float(max(1, len(accept_flags)))
    mean_est = np.mean(samples[burnin:], axis=0)

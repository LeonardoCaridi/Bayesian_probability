...
def log_prior(theta, bounds):
    ...
    # Non necessario: sigma viene presa costante in log(sigma)
...
def log_T90_distribution(x, theta, sigma_logT90 = 0.0):
    ...
    - theta: parametri distribuzione [w, mu1, delta, ln_sigma1, ln_sigma2]
    - sigma_logT90: eventuale errore gaussiano su log(T90)
    """

    w, mu1, delta, ln_s1, ln_s2 = theta

    # sigma in scala lineare
    sigma1 = np.exp(ln_s1)
    sigma2 = np.exp(ln_s2)
    
    if (w <= 0) or (w >= 1) or (delta < 0): return -np.inf
    mu2    = mu1 + delta
...
def log_posterior(x, theta, bounds, sigma_logT90 = 0.0):
    ...
    if lp == -np.inf:
        return -np.inf
    ll = log_likelihood(x, theta, sigma_logT90 = sigma_logT90)
    return lp + ll
...
def metropolis_hastings(theta0, x, bounds, init_cov=None, rng = None, sigma_logT90 = 0.0, n = 1000):
    ...
    for i in range(n):   
        ...
        if(i % (n*0.05) == 0):
            print("iteration {0}: acceptance {1}".format(i,accepted/(i+1)))
...
def init_theta_from_data(x, bounds):
    ...
    theta0 = np.array([np.clip(w0, bounds[0][0]+1e-6, bounds[0][1]-1e-6),
                       ...
                       np.clip(lns1_0, bounds[3][0], bounds[3][1]),
                       np.clip(lns2_0, bounds[4][0], bounds[4][1])], dtype=np.float64)
    return theta0
